...
sub check_after_update {
    ...
    assert_script_run('semanage port -l | grep -E "(^SELinux Port Type|http_port_t).*8888"');
    assert_script_run('diff -q semodule_list_before_migration.txt <(semodule -l)');
    assert_script_run('diff -q semanage_booleans_before_migration.txt <(semanage boolean -l)');
    assert_script_run('diff -q semanage_login_before_migration.txt <(semanage login -l)');
