...
sub initial_state {
    ...
    if (is_microos) {
        ...
        zypper_call('--no-gpg-checks install selinux-policy-targeted selinux-policy-targeted-gaming policycoreutils policycoreutils-python-utils setools-console podman selinux-policy-targeted-gaming');
    }
...
sub update_system {
    ...
    if (is_microos) {
        ...
        zypper_call('dup --force-resolution  --no-confirm');
    }
...
sub check_after_update {
    ...
    assert_script_run('semanage login -l | grep testselinux');
    assert_script_run('semanage port -l | grep -E "(^SELinux Port Type|http_port_t).*8888"');
    script_run('diff  semodule_list_before_migration.txt <(semodule -l)');
    script_run('diff  semanage_booleans_before_migration.txt <(semanage boolean -l)');
    script_run('diff  semanage_login_before_migration.txt <(semanage login -l)');
...
sub check_custom_policy {
    ...
    if (is_microos) {
        ...
        script_run("zypper --no-gpg-checks -n in -y $package_list", timeout => 600);
    }
...
sub cleanup_test_artifacts {
    ...
    script_output("ausearch -m AVC");
    script_output("audit2allow -a");
