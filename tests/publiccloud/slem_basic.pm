...
sub check_avc {
    ...
    #$instance->ssh_script_run(cmd => 'sudo audit2allow -a -M testmodule');
    #$instance->ssh_script_run(cmd => 'sudo semodule -i testmodule.pp');
    $instance->ssh_script_run(cmd => 'sudo systemctl is-active wicked');
    $instance->ssh_script_run(cmd => 'sudo systemctl is-active NetworkManager');
    $instance->ssh_script_run(cmd => 'sudo  ps -eZ | grep NetworkManager*');
    $instance->ssh_script_run(cmd => 'sudo   ps -eZ | grep netconfig*');
    $instance->ssh_script_run(cmd => 'sudo   ps -eZ | grep cloud-netconfig*');
    $instance->ssh_script_run(cmd => 'sudo   ps -eZ | grep network.service*');
    $instance->ssh_script_run(cmd => 'sudo   ps -eZ | grep fd*');
    ...
    $instance->ssh_script_run(cmd => 'sudo ls -Z /run/netconfig.pid');
    $instance->ssh_script_run(cmd => 'sudo ls -Z /run/netconfig.pid');
...
sub run {
    ...
    $instance->run_ssh_command(cmd => 'sudo transactional-update -n pkg install policycoreutils-python-utils restorecond policycoreutils-devel', timeout => 600);
    $instance->softreboot();
